...
Regions are comprised of Towns, Adventure Locations (see Locations in Chapter 10: Adventures), 
...
\textit{Traveling Routes} connect two locations (i.e., Towns, Adventure Locations), 
...
\subsection{Rations}
For each day of travel, each character must expend one day's worth of \textit{Rations},
or else they are inflicted with one level of Exhaustion 
(see Exhaustion under Conditions in Chapter 3: Combat).
Rations may be purchased in a Town and carried in a player's inventory, 
or may be foraged from the environment if the character is able.


\subsection{Foraging}
If a character can forage, then for each day of travel, 
they may roll on the \textit{Foraging Table} (\Roll{d10} or \Roll{d100}) for that route.
Foraging Tables may be exclusive or cumulative and contain potion materials 
and rations depending on the regional geography and ecology.

\begin{Example}
  A route that is tropical or temperate (and therefore ecologically rich) may have a wide variety
  of animals, or roots, berries, and leaves that suffice for either rations or potion ingredients,
  whereas a route in a region that is cold and rocky may have proportionally fewer animals and plants 
  for rations and materials, but a greater abundance of mineral materials.
\end{Example}
...
Additionally, for each day of travel, the traveling character or party of characters 
must roll on the \textit{Encounter Table} (\Roll{d10} or \Roll{d100}) for that route. 
Encounter Tables describe the challenges the party will face while traveling along the route.
