...
(see Factions \& Loyalty in Chapter 9: Regions).

\vspace{0.4em}

When a character is created, you may choose a religion that is appropriate to their background.

\subsection{Deity}
The \textit{Deity} of a religion may grant unique Perks and Abilities to their followers,
or may provide unique buffs from \textit{Prayer} when a follower levels-up in a Town with a 
...
\begin{Example}
  ...
  by having them become Unconscious and forcing them to succeed on a number of 
  ``Death Saves'' in order to maintain that state.
\end{Example}
