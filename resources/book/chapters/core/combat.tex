...
\subsection{Cast Spell}
In order to use \textit{Cast Spell}, a character must be equipped with a Mana Focus (see Weapons: Mana Focus in Chapter 8: Equipment).

\vspace{0.4em}

To cast a spell, choose a Spell (see Abilities: Spells in Chapter 5: Perks, Abilities, and Classes),
then expend Mana equal to that spell's Mana cost. 

\vspace{0.4em}

This ability's effect is the effect of the spell that is cast.
...
this effect becomes can only be removed when they level-up in a Town with a Hospital.
Otherwise, this effect is removed when the character is restored to their maximum Life.

\vspace{0.4em}

If a body part is inflicted with Crippled a third time, the limb is \textit{Severed}.
A severed arm or leg is permanently Crippled. 
A character with a severed torso or head is killed.
...
All of the character's actions gain an additional Mana cost of 1.
